...
namespace ConaviWeb.Data.Levantamientos
{
    public class LevantamientoRepository : ILevantamientoRepository
    {
        ...
        public async Task<IEnumerable<Catalogo>> GetEstados()
        {
            var db = DbConnection();
            var sql = @"
                        select clave Clave,descripcion Descripcion from prod_control_exp.c_entidad_federativa where clave != '00';
                       ";
        ...
        public async Task<IEnumerable<Catalogo>> GetMunicipios(string cveedo)
        {
            var db = DbConnection();
            var sql = @"
                        select clave Clave,descripcion Descripcion from prod_control_exp.c_municipio where clave_entidad_federativa = @CveEdo and clave != '000';
                       ";
        ...
        public async Task<IEnumerable<Catalogo>> GetLocalidades(string cveedo, string cvemun)
        {
            var db = DbConnection();
            var sql = @"
                        select clave Clave,descripcion Descripcion from prod_control_exp.c_localidad where clave_entidad_federativa = @CveEdo and clave_municipio = @CveMun and clave != '0000';
                       ";
