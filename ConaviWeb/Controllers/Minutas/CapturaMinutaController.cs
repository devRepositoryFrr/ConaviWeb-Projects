...
namespace ConaviWeb.Controllers.Minutas
{
    public class CapturaMinutaController : Controller
    {
        ...
        [HttpPost, ActionName("GetReunion")]
        public async Task<JsonResult> GetReunionAsync(int idReunion)
        {
            var reunion = await _minutaRepository.GetReunionDetail(idReunion);
        ...
        public async Task<IActionResult> CreateMinutaAsync(Minuta minuta)
        {
            ...
            _ = MinutaPdfAsync(minuta.IdReunion);
        ...
        public async Task MinutaPdfAsync(int id)
        {
            ...
            PdfDocument pdfDoc = new PdfDocument(new PdfWriter(Path.Combine(pathCarta, "minuta_" + id + ".pdf")));
            ...
            foreach (var item in acuerdos)
            {
                ...
                tableAcu.AddCell(new Paragraph(i.ToString()))
                    ...
                    .SetTextAlignment(TextAlignment.CENTER)
                    .SetVerticalAlignment(VerticalAlignment.MIDDLE);
                tableAcu.AddCell(new Paragraph(String.Concat(item.Interno, " | ", item.Externo))).SetFont(fonte).SetFontSize(8);
